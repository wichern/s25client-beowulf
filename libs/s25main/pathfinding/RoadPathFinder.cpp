                                           ...
                                           RoadPathDirection* const firstDir, MapPoint* const firstNodePos)
{
    ...
    */
    ...
    dstarlite::Search search = {gwb_, goal, gwb_.GetEvMgr().GetCurrentGF(), addCosts, isSegmentAllowed};

    dstarlite::Node& startDNode = search.GetNode(start);
    if(startDNode.rhs == std::numeric_limits<unsigned>::max())
    {
        if(!search.ComputeShortestPath(start, max))
    ...
    const noRoadNode* next;
    Direction dir;
    const unsigned rhs = search.GetMinCostSuccessor(start, &next, &dir);
    if(rhs > max)
        return false;
    ...
    if(length)
        *length = startDNode.g;
    if(firstDir)
        *firstDir = toRoadPathDirection(dir);
    if(firstNodePos)
        *firstNodePos = next->GetPos();
    return true;
}
