...
World::World(Beowulf* beowulf, bool fow)
    : resources(beowulf->GetAII(), *this, fow), beowulf_(beowulf), aii_(beowulf->GetAII()), fow_(fow)
{
    ...
    for(BuildingType type : helpers::enumRange<BuildingType>())
        if(BuildingProperties::IsUsual(type))
            for(const nobUsual* bld : aii_.GetBuildings(type))
                SetPoint(Create(bld->GetBuildingType(), Building::Finished), bld->GetPos());
        ...
        for(const RoadDir rdir : helpers::EnumRange<RoadDir>{})
        {
            if(aii_.gwb.GetRoad(pt, rdir) != PointRoad::None)
            {
                if(aii_.gwb.GetNO(pt)->GetType() != NodalObjectType::Building
                   && aii_.gwb.GetNO(GetNeighbour(pt, OppositeDirection(toDirection(rdir))))->GetType()
    ...
    eventSubscriptions_.push_back(notifications.subscribe<BuildingNote>(
      boost::lambda::if_(boost::lambda::bind(&BuildingNote::player, boost::lambda::_1)
                         == aii_.GetPlayerId())[boost::lambda::bind(&World::OnBuildingNote, this, boost::lambda::_1)]));
    eventSubscriptions_.push_back(notifications.subscribe<RoadNote>(
      boost::lambda::if_(boost::lambda::bind(&RoadNote::player, boost::lambda::_1)
                         == aii_.GetPlayerId())[boost::lambda::bind(&World::OnRoadNote, this, boost::lambda::_1)]));
    eventSubscriptions_.push_back(notifications.subscribe<FlagNote>(
      boost::lambda::if_(boost::lambda::bind(&FlagNote::player, boost::lambda::_1)
                         == aii_.GetPlayerId())[boost::lambda::bind(&World::OnFlagNote, this, boost::lambda::_1)]));
}
...
void World::PlanRoad(const MapPoint& pt, const std::vector<Direction>& route)
{
    MapPoint cur = pt;
...
void World::ClearPlan()
{
    ...
    // Clear planned flags.
    flags_.erase(
      std::remove_if(flags_.begin(), flags_.end(), [&](const MapPoint& pt) { return nodes_[pt].flagPlanCount > 0; }),
      flags_.end());
...
void World::RemoveRoad(const MapPoint& pt, const std::vector<Direction>& route)
{
    ClearPlan();
    SetRoadState(pt, route, RoadDoesNotExist);
}

Building* World::Create(BuildingType type, Building::State state, unsigned group, const MapPoint& pt)
{
    ClearPlan();
...
bool World::HasRoad(const MapPoint& pt, Direction dir) const
{
    ...
    {
        nodePt = pt;
        idx = static_cast<uint8_t>(OppositeDirection(dir));
    } else
    {
        nodePt = GetNeighbour(pt, dir);
        idx = static_cast<uint8_t>(dir);
    }
...
RoadState World::GetRoadState(const MapPoint& pt, Direction dir) const
{
    ...
    if(static_cast<uint8_t>(dir) >= 3)
        return nodes_[pt].roads[static_cast<uint8_t>(oppositeDir)];
    else
        return nodes_[GetNeighbour(pt, dir)].roads[static_cast<uint8_t>(dir)];
}
...
void World::SetRoadState(const MapPoint& pt, Direction dir, RoadState state)
{
    Direction oppositeDir = OppositeDirection(dir);
    if(static_cast<uint8_t>(dir) >= 3)
        nodes_[pt].roads[static_cast<uint8_t>(oppositeDir)] = state;
    else
        nodes_[GetNeighbour(pt, dir)].roads[static_cast<uint8_t>(dir)] = state;
}

void World::SetRoadState(const MapPoint& pt, const std::vector<Direction>& route, RoadState state)
{
    MapPoint cur = pt;
    for(size_t i = 0; i < route.size(); ++i)
    {
...
BuildingQuality World::GetBQ(const MapPoint& pt, bool includeAnticipated,
...
bool World::IsRoadPossible(const MapPoint& pt, Direction dir, bool includeAnticipated,
                           const std::vector<std::pair<MapPoint, BuildingQuality>>& tmps) const
{
...
std::vector<Direction> World::GetPath(const MapPoint& src, const MapPoint& dst) const
{
    std::vector<Direction> ret;
    FindPath(
      src, *this, &ret, [this](const MapPoint& pt, Direction dir) { return HasRoad(pt, dir); }, // Condition
...
std::pair<MapPoint, unsigned> World::GetNearestBuilding(const MapPoint& pt, const std::vector<BuildingType>& types,
...
Building* World::GetGoodsDest(const Building* building, const MapPoint& pt) const
{
...
World::ProductionGroup& World::CreateGroup(const std::vector<BuildingType>&& types, const MapPoint& region)
{
...
bool World::CanConnectBuilding(const MapPoint& buildingFlag, const MapPoint& dst, bool includeAnticipated,
                               const std::vector<std::pair<MapPoint, BuildingQuality>>& tmps) const
{
    if(buildingFlag == dst)
        return true;
    ...
    return FindPath(
      ...
      // Heuristic
      [&](const MapPoint& pt) { return CalcDistance(pt, dst); },
      // Cost
      [&](const MapPoint&, Direction) { return 1; });
}
...
bool World::IsConnected(const MapPoint& src, const MapPoint& dst) const
{
    return FindPath(
      src, *this, nullptr,
      // Condition
      [&](const MapPoint& pt, Direction dir) { return HasRoad(pt, dir); },
      // End
      [&](const MapPoint& pt) {
          // The search can end if we found a way to any flag of the destination
          // road network.
...
unsigned World::GetEnemySoldiersInReach(const MapPoint& pt) const
{
    ...
    for(const nobBaseMilitary* bld : blds)
    {
        ...
        {
            case BuildingType::Barracks: ret += 2 - 1; break;
            case BuildingType::Guardhouse: ret += 3 - 1; break;
            case BuildingType::Watchtower: ret += 6 - 1; break;
            case BuildingType::Fortress: ret += 9 - 1; break;
...
void World::PredictExpansionResults(const MapPoint& pt, BuildingType type, std::vector<MapPoint>& additionalTerritory,
...
{
    if(static_cast<uint8_t>(dir) >= 3)
        nodes_[pt].roadPlanCount[static_cast<uint8_t>(OppositeDirection(dir))]++;
    else
        nodes_[GetNeighbour(pt, dir)].roadPlanCount[static_cast<uint8_t>(dir)]++;
}
...
BlockingManner World::GetBM(const MapPoint& pt, const std::vector<std::pair<MapPoint, BuildingQuality>>& tmps) const
{
    const Node& node = nodes_[pt];
    if(node.building)
    ...
    {
        Building* building = nodes_[GetNeighbour(pt, dir)].building;
...
BuildingQuality World::AdjustBQ(const MapPoint pt, BuildingQuality nodeBQ, bool includeAnticipated) const
{
       ...
       && !IsPlayerTerritory(GetNeighbour(pt, Direction::SouthEast), includeAnticipated))
    {
        ...
        for(Direction dir = Direction::West; dir <= Direction::NorthEast; dir += 1U)
        {
            if(GetBM(GetNeighbour(pt, dir)) == BlockingManner::Flag)
                return BuildingQuality::Nothing;
        }
...
std::pair<MapPoint, unsigned> World::GroupMemberDistance(const MapPoint& pt, unsigned group, BuildingType type) const
{
...
unsigned World::GetMaxGroupMemberDistance(const MapPoint& pt, unsigned group, BuildingType type) const
{
    if(group == InvalidProductionGroup)
