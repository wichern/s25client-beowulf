...
CoinManager::CoinManager(Beowulf* beowulf) : RecurrentBase(beowulf, 10) {}
...
void CoinManager::RequestAcademy()
{
    ...
    if(bestLocation.isValid())
    {
        if(!academy_)
        {
            academy_ = beowulf_->world.Create(BuildingType::Fortress, Building::ConstructionRequested,
                                              InvalidProductionGroup, bestLocation);
        } else
        {
