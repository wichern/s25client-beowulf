...
ProductionPlanner::Region::Region(const Building* building, const MapPoint& flag, bool main)
    : isMain(main), regionFlag(flag), buildings{building}
{
    resources.fill(0);
}
...
unsigned ProductionPlanner::Region::CountBuildings(const std::vector<BuildingType>&& types,
                                                   bool constructionFinished) const
{
    return static_cast<unsigned>(std::count_if(buildings.begin(), buildings.end(), [&](const Building* building) {
...
ProductionPlanner::ProductionPlanner(Beowulf* beowulf) : RecurrentBase(beowulf, 15, 0) // Plan only every 15*16 gf.
{}
...
void ProductionPlanner::Plan(const MapPoint& regionPt, Region& region)
{
    ...
    unsigned militaries = region.CountBuildings(
      {BuildingType::Barracks, BuildingType::Guardhouse, BuildingType::Watchtower, BuildingType::Fortress}, true);
    static const std::vector<unsigned> fib = {1, 2, 3, 5, 8, 13, 21, 34};
    if(sawmills < 2 || militaries > fib[sawmills])
    {
        World::ProductionGroup& group = beowulf_->world.CreateGroup(
    ...
    if(region.isMain)
    {
        ...
        {
            World::ProductionGroup& group =
              beowulf_->world.CreateGroup({BuildingType::Ironsmelter, BuildingType::Armory}, regionPt);
    ...
    if(region.production[BGD_GRAIN].produced > region.production[BGD_GRAIN].consumed)
    {
        if(0 == beowulf_->build.GetRequestCount({BuildingType::Bakery, BuildingType::Slaughterhouse}, regionPt))
        {
            ...
            unsigned possibleBakers =
              region.GetTotalJobs(Job::Baker) + region.GetTotalGoods(*JOB_CONSTS[Job::Baker].tool);
            unsigned possibleButchers =
              region.GetTotalJobs(Job::Butcher) + region.GetTotalGoods(*JOB_CONSTS[Job::Butcher].tool);
            if(possibleBakers > 0 || 0 == possibleButchers)
            {
                World::ProductionGroup& group =
                  beowulf_->world.CreateGroup({BuildingType::Mill, BuildingType::Bakery}, regionPt);
            ...
            {
                World::ProductionGroup& group =
                  beowulf_->world.CreateGroup({BuildingType::Slaughterhouse, BuildingType::PigFarm}, regionPt);
    ...
    if(region.isMain && 0 == region.GetTotalJobs(Job::PackDonkey))
    {
    ...
    if(beowulf_->metalworks.JobOrToolOrQueueSpace(Job::Hunter))
    {
    ...
    if(beowulf_->metalworks.JobOrToolOrQueueSpace(Job::Fisher))
    {
    ...
    int coalOverproduction =
      static_cast<int>(region.production[BGD_COAL].produced) - static_cast<int>(region.production[BGD_COAL].consumed);
    int ironOverproduction =
      static_cast<int>(region.production[BGD_IRON].produced) - static_cast<int>(region.production[BGD_IRON].consumed);
    int goldOverproduction =
      static_cast<int>(region.production[BGD_GOLD].produced) - static_cast<int>(region.production[BGD_GOLD].consumed);
    ...
    if(coalOverproduction < ironOverproduction && coalOverproduction < goldOverproduction
       && region.resources[BResourceCoal] && 0 == requestedCoal)
    {
        RequestBuilding(regionPt, BuildingType::CoalMine);
        if(0 == --maxRequests)
            return;
    } else if(ironOverproduction < coalOverproduction && ironOverproduction < goldOverproduction
              && region.resources[BResourceIron] && 0 == requestedIron)
    {
        RequestBuilding(regionPt, BuildingType::IronMine);
        if(0 == --maxRequests)
            return;
    } else if(goldOverproduction < coalOverproduction && goldOverproduction < ironOverproduction
              && region.resources[BResourceGold] && 0 == requestedGold)
    {
...
void ProductionPlanner::RequestBuilding(const MapPoint& regionPt, BuildingType type, unsigned group)
{
...
void ProductionPlanner::CalculateRegions()
{
    ...
    if(nullptr == hq)
    {
        ...
        for(auto& it : regions_)
        {
            ...
            {
                if(bld->GetType() == BuildingType::Metalworks || bld->GetType() == BuildingType::Armory
                   || bld->GetType() == BuildingType::Ironsmelter)
                {
                    region.isMain = true;
                    found = true;
                    break;
...
void ProductionPlanner::CalculateRegion(const MapPoint& flag, std::set<const Building*>& considered)
{
    ...
    FloodFill(
      world, flag,
      // Condition
      [&](const MapPoint& pt, Direction dir) { return world.HasRoad(pt, dir); },
      // Action
      [&](const MapPoint& pt) {
          if(world.HasFlag(pt))
          {
    ...
    for(const MapPoint& pt : bl.Get())
    {
        ...
        if(bq == BuildingQuality::Mine)
        {
            ...
            region.resources[BResourceGranite] +=
              world.resources.GetReachable(pt, BResourceGranite, false, true, false);
        } else if(bq >= BuildingQuality::Hut && bq <= BuildingQuality::Castle)
        {
            ...
            region.resources[BResourceHuntableAnimals] +=
              world.resources.GetReachable(pt, BResourceHuntableAnimals, false);
